...
// Define parameters for the C-LOOK algorithm
type CLookParams = {
...
// Define the state for the C-LOOK algorithm
type CLookState = State & {
  diskSize: number;
...
export function simulateStep(state: CLookState): { nextState: CLookState; event: Event } {
  let nextState = { ...state };
  ...
  if (nextState.pendingRequests.length > 0) {
    // C-LOOK: find the next request in the current direction
    ...
    if (nextRequest) {
      ...
      if (nextState.pendingRequests.length > 0) {
        ...
        // For C-LOOK, after reversing direction, we move to the closest request in the new direction.
        // If there are no requests in the new direction, the head remains idle until a new request arrives or the direction changes again.
        let closestRequestInNewDirection: DiskRequest | null = null;
        let closestRequestIndexInNewDirection = -1;
        let minSeekDistanceInNewDirection = Infinity;

        // Filter requests for the new direction
        const requestsInNewDirection = nextState.pendingRequests.filter(req => {
          if (nextState.currentDirection === "increasing") {
            return req.track >= nextState.currentHeadPosition;
          } else { // "decreasing"
            return req.track <= nextState.currentHeadPosition;
        ...
        if (requestsInNewDirection.length > 0) {
          // Sort these requests by track number according to the new direction
          requestsInNewDirection.sort((a, b) => {
            if (nextState.currentDirection === "increasing") {
              return a.track - b.track;
            } else {
              return b.track - a.track;
          ...
          // The first request in the sorted list is the next one to serve
          closestRequestInNewDirection = requestsInNewDirection[0];
          closestRequestIndexInNewDirection = nextState.pendingRequests.findIndex(req => req.id === closestRequestInNewDirection!.id);
        }

        if (closestRequestInNewDirection) {
          const requestToServe = closestRequestInNewDirection;
          const targetTrack = requestToServe.track;
          ...
          event = {
            tick: nextState.tick,
            type: "head-move",
            payload: { from: state.currentHeadPosition, to: nextState.currentHeadPosition, seekDistance: movement, reversing: true },
          };

          // Remove the served request from pending
          nextState.pendingRequests.splice(closestRequestIndexInNewDirection, 1);
          // Mark request as served
          nextState.servedRequests.push(requestToServe);
          // Record the served event
          event = { tick: nextState.tick, type: "served", payload: { requestId: requestToServe.id, track: targetTrack } };
        } else {
          // No requests in the new direction either, head remains idle for this tick
...
export function runToCompletion(state: CLookState, onEvent?: (e: Event) => void): { timeline: Event[]; finalState: CLookState; metrics: any } {
...
// Calculate metrics
function calculateMetrics(state: CLookState) {
  const metrics = {
...
export function getMetrics(state: CLookState): any {
  return calculateMetrics(state);
}

// Serialize state for storage
export function serializeState(state: CLookState): any {
  return JSON.stringify(state);
}

// Deserialize state from storage
export function deserializeState(serialized: any): CLookState {
...
algorithms/disk/c-look.ts

# Visual Studio Code Open Tabs
os-lab-interactive/.env
algorithms/memory/first-fit.ts
algorithms/page-replacement/optimal.ts
algorithms/disk/fcfs.ts
algorithms/disk/sstf.ts
algorithms/disk/scan.ts
../../../../AppData/Roaming/Code/User/globalStorage/saoudrizwan.claude-dev/tasks/1761128774626/focus_chain_taskid_1761128774626.md
algorithms/disk/c-scan.ts
algorithms/disk/look.ts
algorithms/disk/c-look.ts

# Current Time
10/22/2025, 4:15:15 PM (Asia/Calcutta, UTC+5.5:00)

# Context Window Usage
190,139 / 1,000K tokens used (19%)
