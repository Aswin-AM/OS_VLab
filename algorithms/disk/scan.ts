...
// Define parameters for the SCAN algorithm
type ScanParams = {
...
// Define the state for the SCAN algorithm
type ScanState = State & {
  diskSize: number;
...
export function simulateStep(state: ScanState): { nextState: ScanState; event: Event } {
  let nextState = { ...state };
  ...
  if (nextState.pendingRequests.length > 0) {
    // SCAN: find the next request in the current direction
    ...
    if (nextRequest) {
      ...
      if (nextState.pendingRequests.length > 0) {
        ...
        // Move head to the end in the new direction (or to the closest request in the new direction)
        // For simplicity, let's assume it moves to the end of the disk in the new direction if no requests are immediately available.
        // A more accurate simulation would move to the closest request in the new direction.
        // For now, let's simulate a move to the end of the disk to change direction.
        const endTrack = nextState.currentDirection === "increasing" ? nextState.diskSize - 1 : 0;
        const movement = Math.abs(endTrack - nextState.currentHeadPosition);
        nextState.totalHeadMovement += movement;
        nextState.currentHeadPosition = endTrack;

        event = {
          tick: nextState.tick,
          type: "head-move", // This is a move to reverse direction
          payload: { from: state.currentHeadPosition, to: nextState.currentHeadPosition, seekDistance: movement, reversing: true },
        };
        // After reversing, the next step will pick up requests in the new direction.
        // We don't serve a request in this tick, just change direction and move.
        // The loop will continue, and the next iteration will find requests in the new direction.
      } else {
...
export function runToCompletion(state: ScanState, onEvent?: (e: Event) => void): { timeline: Event[]; finalState: ScanState; metrics: any } {
...
// Calculate metrics
function calculateMetrics(state: ScanState) {
  const metrics = {
...
export function getMetrics(state: ScanState): any {
  return calculateMetrics(state);
}

// Serialize state for storage
export function serializeState(state: ScanState): any {
  return JSON.stringify(state);
}

// Deserialize state from storage
export function deserializeState(serialized: any): ScanState {
