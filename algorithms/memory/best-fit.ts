...
// Define the input structure for the Best Fit algorithm
type BestFitInput = {
  memorySize: number;
...
// Define parameters for the Best Fit algorithm
type BestFitParams = {
  // No specific parameters for Best Fit itself
};

// Define the state for the Best Fit algorithm
type BestFitState = State & {
  memorySize: number;
  ...
  pendingRequests: BestFitInput['requests']; // Requests yet to be processed
  // Type for completedRequests to properly include optional allocated properties
  completedRequests: Array<BestFitInput['requests'][0] & { allocatedStart?: number; allocatedSize?: number }>; // Requests that have been processed
  currentTime: number;
...
export function init(
  ...
  const initialState: BestFitState = {
    tick: 0,
...
export function simulateStep(state: BestFitState): { nextState: BestFitState; event: Event } {
  let nextState = { ...state };
  ...
  releasingRequests.forEach((req: BestFitInput['requests'][0] & { allocatedStart?: number; allocatedSize?: number }) => {
...
export function runToCompletion(state: BestFitState, onEvent?: (e: Event) => void): { timeline: Event[]; finalState: BestFitState; metrics: any } {
...
function calculateMetrics(state: BestFitState) {
  const metrics = {
...
export function getMetrics(state: BestFitState): any {
  return calculateMetrics(state);
}

// Serialize state for storage
export function serializeState(state: BestFitState): any {
  return JSON.stringify(state);
}

// Deserialize state from storage
export function deserializeState(serialized: any): BestFitState {
