...
// Define the input structure for the First Fit algorithm
type FirstFitInput = {
  memorySize: number;
...
// Define parameters for the First Fit algorithm
type FirstFitParams = {
  // No specific parameters for First Fit itself, but could include tie-breaking if needed
};

// Define the state for the First Fit algorithm
type FirstFitState = State & {
  memorySize: number;
  ...
  pendingRequests: FirstFitInput['requests']; // Requests yet to be processed
  // Corrected type for completedRequests to properly include optional allocated properties
  completedRequests: Array<FirstFitInput['requests'][0] & { allocatedStart?: number; allocatedSize?: number }>; // Requests that have been processed
  currentTime: number;
  params: FirstFitParams;
};
...
export function init(
  ...
  const initialState: FirstFitState = {
    tick: 0,
...
export function simulateStep(state: FirstFitState): { nextState: FirstFitState; event: Event } {
  let nextState = { ...state };
  ...
  // Explicitly type 'req' to include optional allocatedStart and allocatedSize properties
  const releasingRequests = nextState.completedRequests.filter(req => req.releaseTime === nextState.currentTime);
  releasingRequests.forEach((req: FirstFitInput['requests'][0] & { allocatedStart?: number; allocatedSize?: number }) => {
  ...
  allocatingRequests.forEach(req => {
    let bestFitBlock: MemoryBlock | null = null;
    let bestFitBlockIndex = -1;
    ...
    if (bestFitBlock) {
      // Allocate memory
      const allocatedStart = bestFitBlock.start;
      const allocatedSize = req.size;
      const remainingSize = bestFitBlock.size - allocatedSize;

      // Update free blocks: remove the used block, add remaining part if any
      nextState.freeBlocks.splice(bestFitBlockIndex, 1); // Remove the block
      if (remainingSize > 0) {
...
export function runToCompletion(state: FirstFitState, onEvent?: (e: Event) => void): { timeline: Event[]; finalState: FirstFitState; metrics: any } {
...
function calculateMetrics(state: FirstFitState) {
  const metrics = {
...
export function getMetrics(state: FirstFitState): any {
  return calculateMetrics(state);
}

// Serialize state for storage
export function serializeState(state: FirstFitState): any {
  return JSON.stringify(state);
}

// Deserialize state from storage
export function deserializeState(serialized: any): FirstFitState {
