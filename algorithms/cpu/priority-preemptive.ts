...
type PriorityPreemptiveInputProcess = {
  pid: number;
  arrivalTime: number;
  burstTime: number;
  priority: number;
...
type PriorityPreemptiveState = State & {
  processes: (PriorityPreemptiveInputProcess & { remainingTime: number })[]; // Processes yet to arrive
  readyQueue: (PriorityPreemptiveInputProcess & { remainingTime: number })[]; // Processes ready to run
  runningProcess: (PriorityPreemptiveInputProcess & { remainingTime: number }) | null; // Currently running process
  completedProcesses: (PriorityPreemptiveInputProcess & { remainingTime: number; completionTime: number })[]; // Processes that have finished
  currentTime: number;
  params: PriorityPreemptiveParams;
};

// Define parameters for this algorithm
type PriorityPreemptiveParams = {
  tieBreak: "arrival" | "priorityHighFirst";
  preempt: true; // Explicitly state preemption is enabled
};

// Helper function to sort the ready queue based on priority and tie-breaking rules
function sortReadyQueue(
  queue: (PriorityPreemptiveInputProcess & { remainingTime: number })[],
  params: PriorityPreemptiveParams
): (PriorityPreemptiveInputProcess & { remainingTime: number })[] {
...
export function init(
  params: PriorityPreemptiveParams,
  input: PriorityPreemptiveInputProcess[]
): PriorityPreemptiveState {
  const initialState: PriorityPreemptiveState = {
    tick: 0,
    ...
    completedProcesses: [],
    currentTime: 0,
    params: { ...params, preempt: true }, // Ensure preempt is true
  };
  return initialState;
}

// Simulate one step of the algorithm
export function simulateStep(state: PriorityPreemptiveState): { nextState: PriorityPreemptiveState; event: Event } {
  let nextState = { ...state };
  ...
  arrivingProcesses.forEach((p: PriorityPreemptiveInputProcess & { remainingTime: number }) => {
  ...
  // 2. Scheduling decision (preemptive)
  const highestPriorityReady = nextState.readyQueue.length > 0 ? nextState.readyQueue[0] : null;

  if (nextState.runningProcess) {
    // Check for preemption
    if (highestPriorityReady && highestPriorityReady.priority < nextState.runningProcess.priority) {
      // Preempt the current process
      const preemptedProcess = { ...nextState.runningProcess };
      nextState.readyQueue.unshift(preemptedProcess); // Add back to ready queue
      nextState.readyQueue = sortReadyQueue(nextState.readyQueue, nextState.params); // Re-sort after adding back
      nextState.runningProcess = null; // CPU is now free

      event = {
        tick: nextState.tick,
        type: "preemption",
        payload: { fromPid: preemptedProcess.pid, toPid: highestPriorityReady.pid, queueId: "ready" },
      };
    }
  }
...
export function runToCompletion(state: PriorityPreemptiveState, onEvent?: (e: Event) => void): { timeline: Event[]; finalState: PriorityPreemptiveState; metrics: any } {
...
function calculateMetrics(state: PriorityPreemptiveState) {
  const metrics = {
  ...
  state.completedProcesses.forEach((p: PriorityPreemptiveInputProcess & { remainingTime: number; completionTime: number }) => {
...
export function getMetrics(state: PriorityPreemptiveState): any {
...
export function serializeState(state: PriorityPreemptiveState): any {
  return JSON.stringify(state);
}

// Deserialize state from storage
export function deserializeState(serialized: any): PriorityPreemptiveState {
...
export function describeEvent(e: Event): string {
  switch (e.type) {
    ...
    case "preemption":
      return `Process ${e.payload.fromPid} was preempted by Process ${e.payload.toPid}.`;
    case "idle":
      return `CPU is idle.`;
    case "noop":
      return `No operation.`;
