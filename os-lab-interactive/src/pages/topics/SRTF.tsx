...
const SRTF = () => {
  ...
  const runSimulation = () => {
    let simProcesses = processes.map(p => ({ ...p, remaining: p.burst, isFinished: false }));
    ...
    while (finishedCount < processes.length) {
      const available = simProcesses.filter(p => p.arrival <= currentTime && !p.isFinished);
      ...
      if (currentProcess.remaining === 0) {
        ...
        waitingTimes[currentProcess.id] = turnaroundTimes[currentProcess.id] - originalProcess.burst;
      }
    }
  ...
  return (
    <div className="min-h-screen py-12">
      <div className="container max-w-7xl">
        <div className="mb-8">
          <h1 className="mb-2 text-4xl font-bold">SRTF - Shortest Remaining Time First</h1>
          <p className="text-muted-foreground">
            Preemptive scheduling where the process with the shortest remaining time is executed.
          </p>
        </div>
